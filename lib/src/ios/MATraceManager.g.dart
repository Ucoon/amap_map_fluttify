...
class MATraceManager extends NSObject  {
        ...
        try {
          ...
          switch (methodCall.method) {
            case 'Callback::MATraceDelegate::traceManager_didTrace_correct_distance_withError':
              ...
              await delegate?.traceManager_didTrace_correct_distance_withError(AmapMapFluttifyIOSAs(args['manager']), (args['locations'] as List)?.map((it) => AmapMapFluttifyIOSAs<CLLocation>(it))?.toList(), (args['tracePoints'] as List)?.map((it) => AmapMapFluttifyIOSAs<MATracePoint>(it))?.toList(), args['distance'], AmapMapFluttifyIOSAs(args['error']));
              break;
          ...
          try {
            ...
            switch (methodCall.method) {
              case 'Callback::MAProcessingCallback::MAProcessingCallback':
                ...
                // handle the native call
                if (processingCallback != null) await processingCallback(args['index'], (args['points'] as List)?.map((it) => AmapMapFluttifyIOSAs<MATracePoint>(it))?.toList());
                break;
              default:
                throw MissingPluginException('方法${methodCall.method}未实现');
                break;
          ...
          try {
            ...
            switch (methodCall.method) {
              case 'Callback::MAFinishCallback::MAFinishCallback':
                ...
                // handle the native call
                if (finishCallback != null) await finishCallback((args['points'] as List)?.map((it) => AmapMapFluttifyIOSAs<MATracePoint>(it))?.toList(), args['distance']);
                break;
              default:
                throw MissingPluginException('方法${methodCall.method}未实现');
                break;
          ...
          try {
            ...
            switch (methodCall.method) {
              case 'Callback::MAFailedCallback::MAFailedCallback':
                ...
                // handle the native call
                if (failedCallback != null) await failedCallback(args['errorCode'], args['errorDesc']);
                break;
              default:
                throw MissingPluginException('方法${methodCall.method}未实现');
                break;
  ...
  @deprecated
  Future<void> startTraceWith(void Function(List<CLLocation> locations, List<MATracePoint> tracePoints, double distance, NSError error) locCallback) async {
    // print log
    if (fluttifyLogEnabled) {
          ...
          try {
            ...
            switch (methodCall.method) {
              case 'Callback::MATraceLocationCallback::MATraceLocationCallback':
                ...
                // handle the native call
                if (locCallback != null) await locCallback((args['locations'] as List)?.map((it) => AmapMapFluttifyIOSAs<CLLocation>(it))?.toList(), (args['tracePoints'] as List)?.map((it) => AmapMapFluttifyIOSAs<MATracePoint>(it))?.toList(), args['distance'], AmapMapFluttifyIOSAs(args['error']));
                break;
              default:
                throw MissingPluginException('方法${methodCall.method}未实现');
                break;
